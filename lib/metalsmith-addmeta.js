		...
		function addNav(nav) {
			...
			for (let c = 0; c < nav.child.length; c++) {
				...
				files[nav.child[c].index].navrel = {
					...
					parent	: nav.url && (!nav.hidden || nav.child[c].hidden) ?
											{ title: nav.title, url: nav.url } :
											null,

					back		: c === 0 || (nav.child[c-1].hidden && !nav.child[c].hidden) ?
											null :
											{ title: nav.child[c-1].title, url: nav.child[c-1].url },

					next		: c+1 === nav.child.length || (nav.child[c+1].hidden && !nav.child[c].hidden) ?
											null :
											{ title: nav.child[c+1].title, url: nav.child[c+1].url },

					child		: nav.child[c].child
											.filter(i => !i.hidden || i.hidden === nav.child[c].hidden)
											.map(i => { return { title: i.title, description: i.desc || null, url: i.url, date: i.date, dateF: i.dateF, tag: i.tag }; })
				};
