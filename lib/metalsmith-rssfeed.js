	...
	return (files, metalsmith, done) => {
		...
		if (page && page.child && page.child.length) {
			...
			for (let c = 0; c < page.child.length && c <= opt.maxitems; c++) {
				...
				content = (extract.length === 2 ? extract[1].trim().replace(/\t/g, '') : '');
				...
				feed +=
					...
					`<category>${opt.include}</category>\n` +
					(cp.desc ? `<description>${cs}${cp.desc}${ce}</description>\n` : '') +
			...
			if (feed) {
				feed =
					...
					(meta.desc ? `<description>${meta.desc}</description>\n` : '') +
